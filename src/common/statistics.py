...
def percentile(values: Iterable[float], pct: float) -> float:
    ...
    if len(data) == 1:
        return data[0]
    rank = (len(data) - 1) * pct / 100.0##插线法求值
    low = math.floor(rank)
    high = math.ceil(rank)
    if low == high:
...
def safe_div(numerator: float, denominator: float, default: float = 0.0) -> float:
    return default if denominator == 0 else numerator / denominator##分母不为0


def summarize(values: Iterable[float]) -> dict[str, float]:##日志数值摘要
...
def cdf_points(values: Iterable[float]) -> list[tuple[float, float]]:##cdf画图
